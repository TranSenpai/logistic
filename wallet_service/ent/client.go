...
func (c *ProcessedMessageClient) UpdateOne(_m *ProcessedMessage) *ProcessedMessageUpdateOne {
	mutation := newProcessedMessageMutation(c.config, OpUpdateOne, withProcessedMessage(_m))
	return &ProcessedMessageUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *ProcessedMessageClient) UpdateOneID(id string) *ProcessedMessageUpdateOne {
...
func (c *ProcessedMessageClient) DeleteOne(_m *ProcessedMessage) *ProcessedMessageDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ProcessedMessageClient) DeleteOneID(id string) *ProcessedMessageDeleteOne {
...
func (c *ProcessedMessageClient) Get(ctx context.Context, id string) (*ProcessedMessage, error) {
	return c.Query().Where(processedmessage.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ProcessedMessageClient) GetX(ctx context.Context, id string) *ProcessedMessage {
...
func (c *ProcessedMessageClient) Hooks() []Hook {
	hooks := c.hooks.ProcessedMessage
	return append(hooks[:len(hooks):len(hooks)], processedmessage.Hooks[:]...)
}
...
func (c *WalletClient) Interceptors() []Interceptor {
	inters := c.inters.Wallet
	return append(inters[:len(inters):len(inters)], wallet.Interceptors[:]...)
}
...
type (
	hooks struct {
		ProcessedMessage, Transaction, Wallet []ent.Hook
	}
	inters struct {
		ProcessedMessage, Transaction, Wallet []ent.Interceptor
	}
)
