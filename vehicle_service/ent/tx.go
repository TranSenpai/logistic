...
func (tx *Tx) init() {
	tx.DriverAvailability = NewDriverAvailabilityClient(tx.config)
	tx.Vehicle = NewVehicleClient(tx.config)
	tx.VehicleDocument = NewVehicleDocumentClient(tx.config)
	tx.VehicleLocation = NewVehicleLocationClient(tx.config)
}
...
// applies a query, for example: DriverAvailability.QueryXXX(), the query will be executed
