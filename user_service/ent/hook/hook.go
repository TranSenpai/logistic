...
// The ShipperProfileFunc type is an adapter to allow the use of ordinary
// function as ShipperProfile mutator.
type ShipperProfileFunc func(context.Context, *ent.ShipperProfileMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ShipperProfileFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ShipperProfileMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ShipperProfileMutation", m)
}
